  ...
  return (
    <FadeIn>
      ...
      <p>
        ...
        Benefit change has two routes. They can build the reform by hand in the web app, setting each
        parameter through the interface and reading the results off the charts it returns. Or they can write
        code against the model: find the parameter path in the catalogue, construct a reform object, run
        baseline and reform over the same survey microdata, then take the budgetary or distributional measure
        from the output. Either way a first pass usually takes several attempts to settle the parameter, the
        date, and the measure.
      </p>
      <p>
        ...
        household working out what a reform would mean for them, the barrier is the setup rather than the
        arithmetic.
      </p>
      <p>
        ...
        anyone can rely on, and what UK Chat puts in its place. UK Chat replaces neither route — custom
        variables, bespoke datasets, and reproducible scripts still belong in code, and the web app still
        gives full manual control — but for the questions the typed tools already cover, it removes the setup
        and leaves the calculation where it was.
      </p>
    </FadeIn>
  );
}
