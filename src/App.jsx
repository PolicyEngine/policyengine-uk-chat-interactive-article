...
const designPrinciples = [
  {
    id: 'computed',
    icon: '01',
    title: 'Computed, never recalled',
    description:
      'The chat does not quote figures from memory. Every quantitative answer is produced by a tool call against the compiled PolicyEngine UK model and reported straight from that run.',
  },
  {
    id: 'typed-surface',
    icon: '02',
    title: 'A typed tool surface',
    description:
      'There is no model-authored Python execution tool. The model reaches the engine through 21 typed tools with declared schemas, so what it can do is a fixed, reviewable list rather than an open door.',
  },
  {
    id: 'handles',
    icon: '03',
    title: 'Opaque, turn-local handles',
    description:
      'Simulations and derivatives are addressed by handle, not by value. The result store belongs to a single chat turn, and the model can neither inspect nor serialise what sits behind a handle.',
  },
  {
    id: 'reference',
    icon: '04',
    title: 'Reference from the engine',
    description:
      'The capability and parameter reference the model works from is generated by the installed engine, not hand-written. Upgrade the country package and the model’s view of the world updates with it.',
  },
  {
    id: 'aggregate',
    icon: '05',
    title: 'Aggregate-only by construction',
    description:
      'The fixed Enhanced FRS dataset is aggregate-only in chat. The serialisation layer rejects raw dataframes outright, so an accidental row-level payload fails closed instead of becoming a tool response.',
  },
];

const stackLayers = [
  { layer: 'Frontend', technology: 'Next.js 15 and React 19 with Mantine UI, deployed on Vercel' },
  { layer: 'Backend', technology: 'FastAPI on Python 3.13, served as a Modal ASGI app' },
  { layer: 'Agent', technology: 'Anthropic Claude models driven through a streaming tool-use loop' },
  { layer: 'Engine', technology: 'policyengine.py with the policyengine-uk country package' },
  { layer: 'Gateway', technology: 'A cheap per-turn pre-pass that classifies and routes each message' },
  { layer: 'Auth and storage', technology: 'Supabase for auth, Postgres for conversation history' },
  { layer: 'Observability', technology: 'Tracing and metrics across the whole turn' },
];
...
const questionKinds = [
  {
    id: 'rules',
    name: 'Rules',
    summary: 'Current policy settings',
    question: 'What’s the personal allowance this year, and where does it start tapering?',
    description:
      'Allowances, rates, thresholds, benefit amounts, limits, and tapers are read out of the model’s parameter tree for the year you ask about — including the taper points that are easy to state from memory and easy to state wrongly.',
    tools: ['search_parameters', 'get_parameter'],
  },
  {
    id: 'households',
    name: 'Households',
...
function BoundaryDiagram() {
  return (
    ...
    >
      <defs>
        <marker id="boundary-arrow" markerWidth="9" markerHeight="9" refX="7" refY="4.5" orient="auto">
          <path d="M0,0 L9,4.5 L0,9 Z" className="flow-arrow" />
        </marker>
      </defs>
...
function DesignPrinciples() {
  const [top, bottom] = [designPrinciples.slice(0, 3), designPrinciples.slice(3)];

  return (
    <FadeIn>
      <h2>Five rules hold the boundary in place</h2>
      <p>
        The stages above describe what happens in one request. These are the constraints that make the same
        thing happen in every request, whatever the model proposes.
      </p>
      ...
      <div className="principles-grid principles-bottom">
        {bottom.map((principle) => (
          <article className="principle-card" key={principle.id}>
            <div className="principle-icon">{principle.icon}</div>
            <h3 className="principle-title">{principle.title}</h3>
            <p className="principle-desc">{principle.description}</p>
      ...
      <p>
        The handle rule is what makes the privacy boundary structural rather than aspirational. A simulation
        returns an opaque identifier and its metadata; it never returns household rows, survey weights, or a
        serialisable simulation object. Survey weighting is applied inside the engine, by the same output
        classes the rest of PolicyEngine uses.
      </p>
    </FadeIn>
  );
}
...
function WhatYouCanAsk() {
  ...
  return (
    <FadeIn>
      ...
      <p>
        Five kinds of question are supported today, and they compose: a thread can move from a rule lookup to
        a household example to a full reform analysis without switching tools.
      </p>
      <div className="iteration-container">
        <p className="iteration-hint">Select a kind of question to see an example</p>
        <div className="iteration-cards tool-cards question-cards">
          {questionKinds.map((kind, index) => (
            <button
              className={`iteration-card ${activeKind.id === kind.id ? 'active' : ''}`}
              key={kind.id}
              type="button"
              onClick={() => setActiveKind(kind)}
            >
              <span className="iteration-num">{index + 1}</span>
              <span className="iteration-title">{kind.name}</span>
              <span className="iteration-subtitle">{kind.summary}</span>
            </button>
          ))}
        </div>
        <div className="iteration-panel">
...
function Stack() {
  return (
    <FadeIn>
      <h2>The stack</h2>
      <p>
        The backend is organised by topic package, so each directory owns one concern and the calculation
        boundary is one of them rather than a convention spread across the codebase.
      </p>
      <div className="worked-result-table-wrap">
        <table className="worked-result-table stack-table">
      ...
      <p>
        <code>GET /version</code> reports the policyengine.py version and the installed UK country package
        version, so any answer can be tied back to the exact engine that produced it.
      </p>
    </FadeIn>
  );
}
